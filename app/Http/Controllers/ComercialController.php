...
class ComercialController extends Controller
{
    public function comercialView()
    {
        ...
        return view('comercio', compact('rol'));

    }
    
    public function index()
    {
        $comerciales = DB::select("EXEC ObtenerComerciales");
    
        return response()->json(['data' => $comerciales]); // Asegurarte de que el DataTable lo entienda
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $resultado = DB::select("EXEC InsertarComercial ?, ?, ?, ?, ?, ?,?", [
                $request->nombre,
                $request->apellido1,
                $request->apellido2,
                $request->ciudad_id,
            ...
            if (!empty($resultado) && isset($resultado[0]->exito)) {
                if ($resultado[0]->exito == 1) {
                    return response()->json(['message' => 'Comercial creado con éxito', 'success' => true]);
                } else {
            ...
            return response()->json([
                'message' => 'Error al crear comercial',
                'success' => false,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function edit($id)
    {
        try {
            ...
            $resultado = DB::select("EXEC ObtenerComercialesId ?", [$id]);
    
            // Verificar si se devolvieron resultados
            if (!empty($resultado)) {
                return response()->json(['data' => $resultado[0], 'success' => true]);
            } else {
                return response()->json(['message' => 'No se encontró el comercial', 'success' => false]);
            }
        } catch (\Exception $e) {
            \Log::error('Error en edit(): ' . $e->getMessage());
            return response()->json(['message' => 'Error al obtener comercial', 'success' => false, 'error' => $e->getMessage()]);
        }
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $resultado = DB::select("EXEC ActualizarComercial ?, ?, ?, ?, ?, ?,?,?", [
                $id,
                $request->nombre,
                $request->apellido1,
                $request->apellido2,
            ...
            if (!empty($resultado) && isset($resultado[0]->exito)) {
                if ($resultado[0]->exito == 1) {
                    return response()->json(['message' => 'Comercial actualizado con éxito', 'success' => true]);
                } else {
                    return response()->json(['message' => 'No se encontró el Comercial', 'success' => false]);
                }
            }
            ...
            return response()->json([
                'message' => 'Error al actualizar Comercial',
                'success' => false,
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            $resultado = DB::select("EXEC EliminarComercial ?, ?", [$id, 0]);
    
            // Verificar si se devolvió un valor
            if (!empty($resultado) && isset($resultado[0]->exito)) {
                if ($resultado[0]->exito == 1) {
                    return response()->json(['message' => 'Comercial eliminado con éxito', 'success' => true]);
                } else {
                    return response()->json(['message' => 'No se encontró el Comercial', 'success' => false]);
                }
            }
            ...
            return response()->json([
                'message' => 'Error al eliminar Comercial',
                'success' => false,
                'error' => $e->getMessage()
            ]);
        }
